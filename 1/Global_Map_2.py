...
for iy in range(len(syear_list)):
    ...
    if var in ['vEtotLLd', 'vQtotLLd']:
        ...
        for WNr in range(1, 6):
    
            ax = fig.add_subplot(2, 3, WNr, projection=ccrs.NorthPolarStereo())
            ax.set_title('WNr '+str(WNr))
            ax.coastlines('110m', alpha=0.5)    
            ax.set_extent([-180, 180, 30, 90], crs=ccrs.PlateCarree())
            
            
            c = ax.contourf(dsn.lon, dsn.lat, dsn[var].sel(WaveNumb= WNr), transform=ccrs.PlateCarree(), cmap= 'RdBu_r', vmin= -d_extreme, vmax= d_extreme) #norm= norm)
        ...
        c = ax.contourf(dsn.lon, dsn.lat, dsn[var].sum(dim= 'WaveNumb'), transform=ccrs.PlateCarree(), cmap= 'RdBu_r', vmin= -2* d_extreme, vmax= 2*d_extreme)
        fig.colorbar(c, ax=ax)
    ...
    if save:
        ...
        file_name= 'Map_M'+str(Member) +'_' + str(syear_list[iy])+'-'+str(eyear_list[iy])+'_'
        file_name += var
...
if var in ['vEtotLLd', 'vQtotLLd']:
    ...
    for WNr in range(1, 6):
        ...
        c = ax.contourf(dsn.lon, dsn.lat, dsn[var].sel(WaveNumb= WNr), transform=ccrs.PlateCarree(), cmap= 'RdBu_r', vmin= -d_extreme/2, vmax= d_extreme/2) #norm= norm)
        fig.colorbar(c, ax=ax)
...
if save:
    ...
    file_name= 'Map_M'+str(Member) +'_Diff' + str(syear_list[-1])+'-'+str(eyear_list[-1])+ 'mi' + str(syear_list[-2])+'-'+str(eyear_list[-2])+'_'
    file_name += var
    savefile= savedir+ file_name
    print(savefile)
    plt.savefig(savefile , bbox_inches='tight') 