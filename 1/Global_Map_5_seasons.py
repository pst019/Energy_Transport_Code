...
ds= ds.resample(time="QS-DEC").mean() #makes the seasonal means
...
for month in [3]:#, 6, 9, 12]:
    dss= ds.resample(time="QS-DEC").mean()

    dss = ds.where(ds['time.month'] == month, drop= True)
    ...
    #    dsn= ds.where(np.logical_and(ds["time.year"] >= syear_list[iy], ds["time.year"] <= eyear_list[iy]  ), drop=True ).mean(dim='time')
        if i == 0:
            dsn= dss.mean(dim='time')
            if dvar == 'conv_Q': levels= [-200, -100, -50, -20, -10, 10, 20, 50, 100, 200] #the first and last are not plotted by necessary for the norm
    ...
    #    if i == 0: dsn= ds.resample(time='1Y').mean().std(dim='time') #annual mean standard deviation
        ...
        if i == 1: 
            dsn1= dss.where(dss["time.year"] >= split_2, drop=True ).mean(dim='time')
            dsn0= dss.where(dss["time.year"] <= split_1, drop=True ).mean(dim='time')
