...
#typ = 'Eddies'
# typ = 'Wavelength_smooth' #not corrected
...
if typ == 'Wavelength_smooth_2': #correct
    Split1= 8000E3
    Split2= 2000E3
...
"""make fraction stationary of total"""    

fig= plt.figure(fignr,  figsize= (6,3))
fignr+=1
plt.clf()

nan_cutoff= 0.1 #if the total transport is less than this fraction of its maximum value (at a latitude) it is set to nan

next(plt.gca()._get_lines.prop_cycler)['color']

#the total - is not really meaning full since the Ferrel cell (wave 0) in the mid latitudes is counteracting the wave transport, but it is stationary.
# plt.plot(ds['lat'], ds[varlist[1]].mean(dim='time').mean(dim='Member').sum(dim='WaveNumb')/ds[varlist[0]].mean(dim='time').mean(dim='Member').sum(dim='WaveNumb'), label='total', color= 'k'  )


ds_n= ds['Plan_'+varlist[0]].mean(dim='time').mean(dim='Member')
plt.plot(ds['lat'], ds['Plan_'+varlist[1]].mean(dim='time').mean(dim='Member').where(np.abs(ds_n) > nan_cutoff *ds_n.max() )/ds['Plan_'+varlist[0]].mean(dim='time').mean(dim='Member'), label='Plan'  )

ds_n= ds['Syno_'+varlist[0]].mean(dim='time').mean(dim='Member')

plt.plot(ds['lat'], ds['Syno_'+varlist[1]].mean(dim='time').mean(dim='Member').where(np.abs(ds_n) > nan_cutoff *ds_n.max() )/ds['Syno_'+varlist[0]].mean(dim='time').mean(dim='Member'), label= 'Syno'  )
plt.xlim(-latcut, latcut)
plt.ylim([0, 1])
plt.legend()

plt.xlabel('Latitude')
plt.ylabel('Fraction of stationary transport \n on the total transport')


if save:

    file_name = model +'_transport_fraction'
    if model == 'EC_Earth': 
        for Member in Memberlist: file_name += '_M'+str(Member)
