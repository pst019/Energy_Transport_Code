...
for ci, cat in enumerate(catlist):
    ...
    for vi, var in enumerate(varlist):    
        ...
        #change in standard deviation
        if vi == 0: axs[1].plot(ds.lat, fac* (ds[cat+'_'+var].where(ds["time.year"]>= split_2).std(dim='time')
                             - ds[cat+'_'+var].where(ds["time.year"]<= split_1).std(dim='time')).mean(dim='Member') , color= color )
        elif cat != 'Meri': axs[1].plot(ds.lat, fac* (ds[cat+'_'+var].where(ds["time.year"]>= split_2).std(dim='time')
                             - ds[cat+'_'+var].where(ds["time.year"]<= split_1).std(dim='time')).mean(dim='Member') , ls= '--', color= color )
...
if save:
    ...
    file_name = 'Change-total+var' +model +'_'
    file_name += typ+'_'+unit 
    if model == 'EC_Earth': 
        for Member in Memberlist: file_name += '_M'+str(Member)
...
"""just a test"""
# fig= plt.figure(fignr, figsize= (8, 3))
# fignr+=1
# plt.clf()
...
# plt.plot( ds.lat, diff_variability)
...
# plt.plot(ds.time, ds[cat+'_'+var].sel(lat= -10, method='nearest'))
