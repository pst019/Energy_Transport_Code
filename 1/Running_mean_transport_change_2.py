...
for vi, var in enumerate(varlist):
    ...
    for mi, member in enumerate(Memberlist):
        ...
        z_scores = np.abs(stats.zscore(ds['tot'].sel(Member= member) ) )    # calculate z-scores
        excludes =(z_scores > zscore_thresh) #.all(axis=1)
        if len(np.where(excludes)[0]) > 0: print('tot', var, 'Member', member, ds.time[excludes].values )
    
        ax0.plot(ds.time[excludes], fac * ds['tot'].sel(Member=member)[excludes], 'o', color= 'k')
    
    excludes =(np.abs(stats.zscore(ds['tot'] )) > zscore_thresh) #.all(axis=1)
    if len(np.where(excludes)[0]) > 0: ds['tot']= ds['tot'].where(~excludes)
        # ds['tot']= ds['tot'].isel(Member= member, time= (z_scores <= zscore_thresh) )
    ...
    if len(Memberlist) > 1:
        mean_var= ds['tot'].rolling(time= 30, min_periods= 30-3, center= True).mean().std(dim='Member')
        std_var= ds['tot'].rolling(time= 30, min_periods= 30-3, center= True).std().std(dim='Member')

        axs[0].fill_between(ds.time, fac*(mean - mean_var), fac*(mean + mean_var), color= line.get_color(), alpha= 0.3)
        axs[1].fill_between(ds.time, fac*(std - std_var), fac*(std + std_var), color= line.get_color(), alpha= 0.3)
...
for ci, cat in enumerate(catlist):
    ...
    for vi, var in enumerate(varlist):    
        ...
        if len(Memberlist) > 1:
            mean_var= ds['var'].rolling(time= 30, min_periods= 30-3, center= True).mean().std(dim='Member')
            std_var= ds['var'].rolling(time= 30, min_periods= 30-3, center= True).std().std(dim='Member')
    
            axs[0].fill_between(ds.time, fac*(mean - mean_var), fac*(mean + mean_var), color= color, alpha= 0.3)
            axs[1].fill_between(ds.time, fac*(std - std_var), fac*(std + std_var), color= color, alpha= 0.3)
