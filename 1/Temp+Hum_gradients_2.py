...
mean_grad= mean_smth.differentiate('lat')/110E3 *1E6 #np.gradient(mean)/dy #divergence
...
mean_grad_sav= scipy.signal.savgol_filter(mean.dropna(dim= 'lat'), window_length=11, polyorder=1, deriv=1)/110E3 *1E6
lat_sav= mean.dropna(dim= 'lat').lat.values


axs[1].plot(ds.lat, mean, '--', label='mean')
axs[1].plot(ds.lat, mean_smth, label=f'mean smooth', color= line.get_color() )

axs[2].plot(ds.lat, mean_grad, ':', color= line.get_color(), label= 'grad mean smooth' )
axs[2].plot(ds.lat, mean_grad_smth, '--', color= line.get_color(), label= 'smooth grad mean smooth' )
axs[2].plot(lat_sav, mean_grad_sav, '-', color= line.get_color(), label= 'savitzky' )



axs[3].plot(ds.lat, trans_mean/mean_grad_smth, '--', label= 'smooth grad mean smooth')
axs[3].plot(lat_sav, trans_mean.sel(lat= lat_sav)/mean_grad_sav, '-', label= 'savitzky', color= line.get_color())
...
line, = axs[0].plot(ds.lat, trans_mean, label= f'{syear}-{eyear}')
line2, =axs[0].plot(ds.lat, trans_mean_last, label=f'{split_2}-{eyear}' )
line3, =axs[0].plot(ds.lat, trans_mean_first, label=f'{syear}-{split_1}' )
...
mean= ds[fvar].mean(dim='time')
# mean_smth= mean.rolling(lat= 5, center= True).mean()
mean_grad_sav= scipy.signal.savgol_filter(mean.dropna(dim= 'lat'), window_length=11, polyorder=1, deriv=1)/110E3/dlat
lat_sav= mean.dropna(dim= 'lat').lat.values

mean_last= ds[fvar].where(ds["time.year"]>= split_2).mean(dim='time') 
# mean_last_smth= mean_last.rolling(lat= 5, center= True).mean()
mean_last_grad_sav= scipy.signal.savgol_filter(mean_last.dropna(dim= 'lat'), window_length=11, polyorder=1, deriv=1)/110E3/dlat


mean_first= ds[fvar].where(ds["time.year"]<= split_1).mean(dim='time') 
# mean_first_smth= mean_first.rolling(lat= 5, center= True).mean()
mean_first_grad_sav= scipy.signal.savgol_filter(mean_first.dropna(dim= 'lat'), window_length=11, polyorder=1, deriv=1)/110E3/dlat
...
axs[2].plot(lat_sav, mean_grad_sav *1E6, '-', color= line.get_color() )
axs[2].plot(lat_sav, mean_last_grad_sav *1E6, '-', color= line2.get_color() )
axs[2].plot(lat_sav, mean_first_grad_sav *1E6, '-', color= line3.get_color() )


axs[3].plot(lat_sav, trans_mean.sel(lat= lat_sav)/mean_grad_sav, '-', color= line.get_color())
axs[3].plot(lat_sav, trans_mean_last.sel(lat= lat_sav)/mean_last_grad_sav, '-', color= line2.get_color())
axs[3].plot(lat_sav, trans_mean_first.sel(lat= lat_sav)/mean_first_grad_sav, '-', color= line3.get_color())
