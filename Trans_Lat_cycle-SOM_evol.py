...
#        t_dat = np.mean(d[bool_array,:,:],axis = 0)
        ...
        for cat in catlist:
            print(str(row) +'/'+ str(col) +' '+ cat ,float(np.mean(ds[cat].where(ds['time.year'] <= eyear_0)[bool_array])/1E6 ), float(np.std(ds[cat].where(ds['time.year'] <= eyear_0)[bool_array])/1E6 ) )
...
if save:
    savedir= '../Figs/SOM_trans-lat-effect/'
