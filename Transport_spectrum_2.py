...
for lat in [60] : #np.arange(-80, 85, 10):
    ...
    ax.plot([-0.5, 0.5, 0.5] + list(dsn.WaveNumb.values[1:]), 2*[dsn[var0].values[0]] + 2*[dsn[var0].values[1]] + list(dsn[var0].values[2:]), color='k')
    ax.fill_between([-0.5, 0.5, 0.5] + list(dsn.WaveNumb.values[1:]), 0, 2*[dsn[var0].values[0]] + 2*[dsn[var0].values[1]] + list(dsn[var0].values[2:]), label= 'Total', alpha=0.5)

    # ax.plot(dsn.WaveNumb, dsn[var0], label='Total')
    ax.plot([-0.5, 0.5, 0.5] + list(dsn.WaveNumb.values[1:]), 2*[dsn[var1].values[0]] + 2*[dsn[var1].values[1]] + list(dsn[var1].values[2:]), color= 'k')
    ax.fill_between([-0.5, 0.5, 0.5] + list(dsn.WaveNumb.values[1:]), 0,  2*[dsn[var1].values[0]] + 2*[dsn[var1].values[1]] + list(dsn[var1].values[2:]), label= 'Stationary', alpha=0.5)
    ...
    ax.text(0, float(dsn[var0].sel(WaveNumb= 0))/2 , str(np.round(100* vEmer/vEtotal, 1))+'%'
            , horizontalalignment='center', verticalalignment='center', rotation='vertical' , weight='bold')    

    # var= 'vEsetot'
    vEseplan= float(dsn[var1].where(np.logical_and(dsn.WaveNumb >= 1, dsn.WaveNumb < WaveSplit1//1)).sum(dim='WaveNumb')+  WaveSplit1%1 * dsn[var1].where(dsn.WaveNumb >= 1).sel(WaveNumb= WaveSplit1//1).fillna(0) )
    ...
    # var= 'vEtot'
    vEteplan= -1*vEseplan + float(dsn[var0].where(np.logical_and(dsn.WaveNumb >= 1, dsn.WaveNumb < WaveSplit1//1)).sum(dim='WaveNumb')+  WaveSplit1%1 * dsn[var0].where(dsn.WaveNumb >= 1).sel(WaveNumb= WaveSplit1//1).fillna(0) )
    ...
    # var= 'vEsetot'
    vEsesyno= float( dsn[var1].where(np.logical_and(dsn.WaveNumb > WaveSplit1//1, dsn.WaveNumb < WaveSplit2//1)).sum(dim='WaveNumb') 
                      + (1- WaveSplit1%1) * dsn[var1].where(dsn.WaveNumb >= 1).sel(WaveNumb= WaveSplit1//1).fillna(0) 
                       + WaveSplit2%1 * dsn[var1].where(dsn.WaveNumb >= 1).sel(WaveNumb= WaveSplit2//1).fillna(0)  )
    ...
    # var= 'vEtot'
    vEtesyno= -1*vEsesyno + float( dsn[var0].where(np.logical_and(dsn.WaveNumb > WaveSplit1//1, dsn.WaveNumb < WaveSplit2//1)).sum(dim='WaveNumb') 
                      + (1- WaveSplit1%1) * dsn[var0].where(dsn.WaveNumb >= 1).sel(WaveNumb= WaveSplit1//1).fillna(0) 
                       + WaveSplit2%1 * dsn[var0].where(dsn.WaveNumb >= 1).sel(WaveNumb= WaveSplit2//1).fillna(0) )
    ...
    # var= 'vEsetot'
    vEsesyno_s= float( dsn[var1].where(np.logical_and(dsn.WaveNumb > WaveSplit2//1, dsn.WaveNumb < WaveSplit3//1)).sum(dim='WaveNumb') 
                      + (1- WaveSplit2%1) * dsn[var1].where(dsn.WaveNumb >= 1).sel(WaveNumb= WaveSplit2//1).fillna(0) 
                       + WaveSplit3%1 * dsn[var1].where(dsn.WaveNumb >= 1).sel(WaveNumb= WaveSplit3//1).fillna(0)  )
    ...
    # var= 'vEtot'
    vEtesyno_s= -1*vEsesyno_s + float( dsn[var0].where(np.logical_and(dsn.WaveNumb > WaveSplit2//1, dsn.WaveNumb < WaveSplit3//1)).sum(dim='WaveNumb') 
                      + (1- WaveSplit2%1) * dsn[var0].where(dsn.WaveNumb >= 1).sel(WaveNumb= WaveSplit2//1).fillna(0) 
                       + WaveSplit3%1 * dsn[var0].where(dsn.WaveNumb >= 1).sel(WaveNumb= WaveSplit3//1).fillna(0) )
    ...
    # var= 'vEtot'
    vEmeso= float(dsn[var0].where(dsn.WaveNumb > WaveSplit3//1).sum(dim='WaveNumb')
                    +  (1-WaveSplit3%1) * dsn[var0].where(dsn.WaveNumb >= 1).sel(WaveNumb= WaveSplit3//1).fillna(0) )
    ...
    ax.text(0, float(dsn[var0].sel(WaveNumb= 0))/2 , str(np.round(100* float(dsn[var0].sel(WaveNumb= 0))/vEtotal, 1))+'%' , horizontalalignment='center', verticalalignment='center', rotation='vertical' , weight='bold')    
