...
d2 = np.reshape(d[varname].data,(numyrs,ndays_yr,len(scales),len(lat)))
dm = np.mean(d2,axis=0)
# Number of days for MA
ndays = 30
ma = bn.move_mean(dm,window=ndays,min_count = 1, axis = 0)
ma = np.roll(ma,-15,axis =0)

temp = d2 - dm
de_seasonalized = np.reshape(temp, (ndays_yr*numyrs,len(scales),len(lat)))
plt.plot(d.time,np.mean(np.mean(de_seasonalized,axis = 1),axis = 1))
plt.plot(d.time,bn.move_mean(np.mean(np.mean(de_seasonalized,axis = 1),axis = 1),window=10,min_count = 1, axis = 0))
...
data.to_netcdf(datadir + 'vQtot_D_deseas.1979-2017.nc')
plt.show()
...
# print(d2)
# d2.to_netcdf(datadir + 'vQtot_D.1979-2017.nc')

#
# first = True
# time = pd.date_range(start = '01/01/1979',end = '01/01/2018',freq='6H')[:-1]
# for year in years:
#     for month in months:
#         nc = nc4.Dataset(f'{datadir}{fname}.{year}.{month}.NS5.nc')
#         if first:
...
# scales = np.array(nc.variables['Scale'])
# # lons = np.array(nc.variables['g0_lon_2'])
# # print(lats.shape)
# # print(lons.shape)
# # print(t_array.shape)
# data = xr.DataArray(t_array, coords = [time,scales,lats], dims =['time','scale','lat'])
# data.name = 'vQtot'
# data.to_netcdf(datadir + 'vQtot_6h.1979-2017.nc')
