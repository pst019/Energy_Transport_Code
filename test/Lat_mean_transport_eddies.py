...
fac = 2* np.pi *6371E3 * np.cos(np.deg2rad(ds.lat))


axs[0].plot(ds.lat, fac*np.sum(ds[var].mean(dim='time') for var in varlist), label= 'Total', color= 'k')
...
for var in varlist:    
    axs[0].plot(ds.lat, fac* ds[var].mean(dim='time'), label= var)

    axs[1].plot(ds.lat, fac* (ds[var].where(ds["time.year"]>= 2050).mean(dim='time') - ds[var].where(ds["time.year"]<= 2000).mean(dim='time')), label= var)
